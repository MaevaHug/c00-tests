...
cc -Wall -Wextra -Werror -o ft_putchar ft_putchar.c main.c
if [ $? -ne 0 ]; then
...
run_test() {
	...
	echo "$separator"
	printf "$> ./ft_putchar \"%q\" | cat -e\n" "$input"

	output=$(./ft_putchar "$input" | cat -e)
...
run_test "b" "b" || all_tests_passed=false
