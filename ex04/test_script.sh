...
cc -Wall -Wextra -Werror -o ft_is_negative main.c ft_is_negative.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_is_negative -1 | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_is_negative $n | cat -e"

	output=$(./ft_is_negative $n | cat -e)
...
run_test 1 "P" || all_tests_passed=false
run_test -1 "N" || all_tests_passed=false
run_test 0 "P" || all_tests_passed=false
